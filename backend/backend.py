...
const API = 'http://localhost:8000'  // đổi thành Render URL khi deploy
...
export default function App() {
  ...
  // Upload file → nhận columns + preview + summary
    ...
    <div className="app">
      ...
      <div className="body">
        ...
        <main className="main">
          <ManualChart
            columns={columns}
            onConfigChange={(cfg) => setChartConfig(cfg)}  // ← MỚI
          />

          // ── MỚI: Data Storytelling ──
          {chartConfig && (
            <StoryPanel
...
.logo  { font-weight: 600; font-size: 14px; color: #534AB7; }
.nav   { display: flex; gap: 4px; flex: 1; margin-left: 8px; }
.nav a { font-size: 12px; color: #666; padding: 5px 10px;
         border-radius: 6px; cursor: pointer; }
.nav a:hover { background: #f5f5f5; }
.quota { font-size: 11px; background: #E1F5EE; color: #085041;
...
export default function ManualChart({ columns, onConfigChange }) {
  ...
  const categoricalCols = columns.filter(c => c.type !== 'numeric')

  // Khi config thay đổi → báo lên App.jsx để StoryPanel nhận
  useEffect(() => {
...
// Màu sắc theo loại insight
const INSIGHT_COLORS = {
  positive: { bg: '#E1F5EE', text: '#085041', border: '#9FE1CB', dot: '#0F6E56' },
  negative: { bg: '#FCEBEB', text: '#A32D2D', border: '#F7C1C1', dot: '#E24B4A' },
  neutral:  { bg: '#EEEDFE', text: '#3C3489', border: '#AFA9EC', dot: '#534AB7' },
}
